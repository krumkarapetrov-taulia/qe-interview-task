describe('Exercise',() => {
        ...
        cy.get('.table-wrapper tbody')
            .then($tbody => {
                ...
                cy.get('[data-testid="modal-header"]')
                    .should("contain", 'Example Modal');
                cy.get('.tau-modal-body').should('contain', 'Hello Jane Doe')
    ...
    it('should not allow user email and role fields to be left empty and give warnings', function () {
        ...
        cy.get('[data-testid="text-role"]')
            .clear()
        cy.get('[data-testid="text-name"]').click();
        cy.get('.tau-field-error')
            .should('contain', 'Required Field');
        cy.get(':nth-child(3) > .tau-field-error')
            .should('contain', 'Required Field');
